...
with open('文件修改二.txt', mode='rt', encoding='utf-8') as read_f\
    ...
    for line in read_f:
        print(line)
        write_f.write(line.replace('夏洛','大海'))
# 文件修改二删除
os.remove('文件修改二.txt')
# 临时文件.txt 改成 文件修改二
...
# 方式二：
# 优点：同一时刻在内存中只存在源文件的一行内容，不会过多占用内存
# 缺点：在文件修改过程中会出现源文件与临时文件共存，硬盘上同一时刻会有两份数据，即
# 在修改的过程中会过多的占用硬盘
